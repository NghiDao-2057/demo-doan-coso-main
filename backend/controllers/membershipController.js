...
export const createMembership = async (req, res) => {
  ...
  try {
    ...
    await sendNotification(
      ...
      `Bạn đã đăng ký thẻ thành viên ${type} thành công! Vui lòng thanh toán để kích hoạt thẻ.`,
      { membershipId: savedMembership._id, type }
    );
...
export const getUserMembership = async (req, res) => {
  try {
    ...
    // Find all memberships for user (active and inactive)
    const memberships = await Membership.find({
      user: userId,
    }).sort({ createdAt: -1 });

    // Return empty array if no membership found (not an error)
